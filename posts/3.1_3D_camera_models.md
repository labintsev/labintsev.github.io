...
Теперь рассмотрим, как использовать камеры-обскуры. Пусть $P = [x y z]^T$ — точка на некотором трёхмерном объекте, видимом для камеры-обскуры. Точка $P$ будет отображена или спроецирована на плоскость изображения $Π'$, в результате чего получится точка $P' = [x' y']^T$.

Аналогично, само точечное отверстие может быть спроецировано на плоскость изображения, что даст новую точку $C'$. Здесь мы можем определить трехмерную систему координат $[i j k]$ с центром отсчета в точке отверстия $O$. Ось k перпендикулярна плоскости изображения и направлена к ней. Эта система координат известна как **система координат камеры**.
...
**Параметры $c_x$ и $c_y$** описывают смещение между координатами плоскости изображения и цифровыми координатами изображения через перенос.  
...
С учётом этого изменения систем координат, проективное отображение точки имеет следующий вид:

$P' = \begin{pmatrix} x' \\\ y' \end{pmatrix} = \begin{pmatrix} \frac{f}{z}x + c_x \\\ \frac{f}{z}y + c_y \end{pmatrix}$ (3)

**Второй важный эффект** — это то, что точки в цифровых изображениях выражаются в пикселях, в то время как точки на плоскости изображения представлены в физических единицах измерений (метрах).

Для учёта этого изменения единиц измерения необходимо ввести два новых параметра $k$ и $l$. Эти параметры имеют размерность [пиксель / милиметр], по двум осям плоскости изображения. Важно отметить, что $k$ и $l$ могут иметь разные значения, поскольку соотношение сторон пикселя не обязательно равно единице. Если $k = l$, мы говорим, что камера имеет квадратные пиксели.  

С учетом этого, отображение выглядит следующим образом:
...
Геометрический смысл параметров $\alpha$ и $\beta$ состоит в том, сколько пикселей будет занимать объект, размер которого на плоскости проекции равен фокусному расстоянию камеры. 

Таким образом, измерение $\alpha$ и $\beta$ в пикселях является необходимым для  работы алгоритмов обработки изображений и взаимосвязи между физическим и цифровым пространством.  
...
Однако на практике гораздо удобне работать с линейными преобразованиями, т.к. их можно представить как произведение входного вектора $P$ на некоторую матрицу. 
Существует ли линейный способ представления проецирования $P \rightarrow P'$ ?  
Ответ - да, представление этого проецирования в виде произведения вектора на матрицу возможно.
...
Для преобразования евклидова вектора $(v_1, ..., v_n)$ в однородные координаты мы просто добавляем 1 в новое измерение, получая $(v_1, ..., v_n, 1)$  Важно отметить, что равенство между вектором и его однородными координатами выполняется только когда последняя координата равна единице. При обратном преобразовании из произвольных однородных координат $(v_1, ..., v_n, w)$ мы получаем евклидовы координаты $(\frac{v_1}{w}, ..., \frac{v_n}{w})$  
...
Основным преимуществом использования однородных координат является то, что они позволяют представить нелинейное преобразование в виде матричного умножения. Это упрощает дальнейшие математические выкладки и обеспечивает единый способ работы с проективными преобразованиями.  

В дальнейшем мы будем работать преимущественно в однородных координатах, если не указано иное. Индекс $h$ опустим, подразумевая, что любая точка $P$ или $P'$ задана в однородных координатах.

Как видно из уравнения (5), можно представить связь между точкой в трёхмерном пространстве и её координатами изображения в виде матрично-векторного соотношения:
...
Полная матричная модель камеры $K$ содержит ключевые параметры $c_x$, $c_y$, $k$ и $l$. 
Однако в текущей формулировке отсутствуют два важных параметра: **скос (skewness)** и **дисторсия (distortion).**
...
Очень часто рассматривается матрица камеры $K$ с 5 степенями свободы:  
- 2 параметра для фокусного расстояния;   
- 2 параметра для смещения;  
- 1 параметр для скоса.   

Однако большинство методов компьютерного зрения игнорируют эффекты дисторсии, позволяя состедоточиться на 4 основных параметрах камеры.
...
Параметры $R$ и $T$ называются **внешними параметрами**, поскольку они определяют положение и не зависят от внутренних характеристик камеры.  
...
- 5 степеней свободы внутренней матрицы камеры;  
- 3 степени свободы внешнего вращения;  
- 3 степени свободы внешнего переноса.   

Таким образом, мы получили полное описание процесса проецирования точки из трёхмерного пространства на плоскость изображения. При этом мы учитываем как характеристики самой камеры, так и её положение в пространстве относительно наблюдаемой сцены.