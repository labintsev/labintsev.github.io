...
Преобразование объектов из реального трёхмерного мира в цифровые изображения требует знания  параметров камеры. В общем случае мы можем как иметь доступ к этим параметрам, так и не иметь его. Однако мы можем вычислить параметры из тех изображений, которые сделаны с помощью этой камеры.  
Задача оценки внешних и внутренних параметров камеры известна как **калибровка камеры**.
...
Рассмотрим этот процесс в контексте калибровочной установки, показанной на рисунке 1.

**Калибровочная установка** обычно состоит из простого шаблона (например, шахматной доски) с известными размерами. Кроме того, установка определяет мировую систему координат с началом $O_w$ и осями $i_w$, $j_w$, $k_w$.
...
Мы составляем линейную систему уравнений из $n$ соответствий, $P_i$, $p_i$ и матрицы камеры $M$, которая состоит из строк $m_1$, $m_2$, $m_3$:

$p_i = \begin{pmatrix} u_i \\\ v_i \end{pmatrix} = MP_i = \begin{pmatrix} \frac{m_1P_i}{m_3P_i} \\\ \frac{m_2P_i}{m_3P_i} \end{pmatrix}$ (2)

Уравнение (2) даёт нам два ограничения для нахождения неизвестных параметров, содержащихся в векторах $m$.

Мы знаем, что матрица камеры имеет 11 неизвестных параметров (6 внешних и 5 внутренних). Это означает, что нам нужно как минимум 6 соответствий для решения. Однако в реальном мире мы часто используем больше соответствий, поскольку измеренные координаты содержат шумы и погрешности.
...
За доказательством данного решения можно обратиться к разделу 5.3 книги Hartley & Zisserman стр. 592–593. 

После преобразования вектора $m$ в матрицу $M$ мы хотим найти внешние и внутренние параметры камеры.
